...
import data_preparation.features_generation.annual_disturbance as annual_disturbance

def process_fire_pipeline(fire_id, disturbance_year):
    """Executes the complete data extraction pipeline for a single fire.

    Args:
      fire_id: the fire ID
      disturbance_year: the year of the DISTURBANCE data
    """
    print(f"\n--- Starting Fire: {fire_id} ---")

    h5_path = Path(settings.H5_OUTPUT_FOLDER) / f"fire_{fire_id}.h5"
    
    # Generate the DISTURBANCE variable
    annual_disturbance.run_single_fire_annual_disturbance(h5_path, 
                                                      annual_disturbance_folder=settings.DISTURBANCE_FOLDER,
                                                      disturbance_year=disturbance_year)

    print(f"--- Finished Fire: {fire_id} ---")


def run_local(all_fire_ids, disturbance_year):
    ...
    for i, fire_id in enumerate(all_fire_ids, start=1):
        ...
        process_fire_pipeline(fire_id, disturbance_year)


def run_distributed(all_fire_ids, disturbance_year, task_id, chunk_size):
...
if __name__ == "__main__":
    ...
    disturbance_year = str(target_year_int - 1)
        
    print(f"Using DISTURBANCE data from year: {disturbance_year}")
    ...
    if args.mode == "local":
        run_local(all_fire_ids, disturbance_year)
    elif args.mode == "distributed":
        if args.task_id == -1:
            print("[!] Error: You must provide a --task-id when using distributed mode.")
            sys.exit(1)
        run_distributed(all_fire_ids, disturbance_year, args.task_id, args.chunk_size)