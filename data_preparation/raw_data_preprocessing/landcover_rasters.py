...
def process_landcover_raster_gdal_90m(input_folder: str, output_folder: str, filename: str, target_crs: str = None):
    """
    Resamples a single LANDCOVER .tif file to 90m using GDAL mode resampling.
    ...
    warp_cmd = [
        ...
        "-r", "mode",         # Calculates the most frequent class in the 3x3 grid
        "-wm", "4000",
        "-multi",
        "-co", "COMPRESS=ZSTD",
        "-co", "TILED=YES",
    ...
    if input_nodata is not None:
        warp_cmd.extend(["-srcnodata", str(input_nodata), "-dstnodata", str(input_nodata)])
...
if __name__ == "__main__":
    ...
    process_landcover_raster_gdal_90m(
        input_folder=settings.LANDCOVER_ORG_FOLDER,
        output_folder=settings.LANDCOVER_FOLDER,
