...
def process_single_raster_gdal_90m(input_folder: str, output_folder: str, year: str, filename: str, target_crs: str = None):
    """
    Resamples a single SCANFI .tif file to 90m using GDAL average resampling.
    Skips processing if the target file already exists in the output folder.
    """
    # Append the year dynamically to the base paths
    input_dir = Path(input_folder) / str(year)
    output_dir = Path(output_folder) / str(year)
    ...
    try:
        ...
        print(f"Success: {output_filename} generated in {year} folder.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--year", type=str, required=True, help="Target year to append to paths (e.g., 2020).")
    parser.add_argument("--filename", type=str, required=True, help="Specific filename to process (e.g., 'tile_01.tif').")
    
    args = parser.parse_args()
    
    process_single_raster_gdal_90m(
        input_folder=settings.SCANFI_ORG_FOLDER,
        output_folder=settings.SCANFI_FOLDER,
        year=args.year,
