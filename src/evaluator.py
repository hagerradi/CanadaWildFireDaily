...
class Trainer:
    ...
    def __init__(
        ...
        self.train_metrics = self._init_metrics(is_train=True)
        self.val_metrics = self._init_metrics(is_train=False)
    ...
    def _init_metrics(self, is_train: bool = False) -> dict[str, nn.Module]:
        ...
        if self.config.use_cumuarea:
            ...
            # ONLY instantiate AUC metrics for Validation
            if not is_train:
    ...
    def _update_metrics(self, metrics_dict: dict[str, nn.Module], predictions: Tensor, masks: Tensor) -> None:
        ...
        if not self.config.use_cumuarea and pred_probs is not None:
            ...
            if 'ap' in metrics_dict:
                metrics_dict['ap'].update(pred_probs, masks)
    ...
    def _compute_and_reset_metrics(self, metrics_dict: dict[str, nn.Module], prefix: str = "") -> dict[str, float]:
        ...
        if self.config.use_cumuarea:
            ...
            if 'aucpr' in metrics_dict:
                ...
                # final_metrics[f"{prefix}AUC_PR"] = auc(recall_np, precision_np)
