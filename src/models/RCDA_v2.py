"""Script for the RCDA Model | Code from https://github.com/hxxAlways/RCDA-Net"""
...
if __name__ == '__main__':
    ...
    model = RCDA(in_ch=55, num_classes=1, depth=4)
    # print(model)
    ...
    input_channels = 55  # 55 inputs channels (terrain, wind, moisture, etc.)
    height, width = 256, 256  # 256x256 pixels (as per paper)

    # Random inputs tensor (simulating 55-channel inputs image)
    x = torch.randn(batch_size, input_channels, height, width)
    # Forward pass
    output = model(x)

    # Check shapes
    print(f"Input shape: {x.shape}")  # Expected: [4, 55, 256, 256]
