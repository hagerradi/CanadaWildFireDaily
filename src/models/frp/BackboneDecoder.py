...
#     the image from the encoded representation back to the original dimensions.
#     """
#     def __init__(self, in_channels, hidden_dims=[256, 128, 64], out_channels=1):
#         super(Decoder, self).__init__()
        
#         # Transposed convolutional layers stepping down the channel dimensions
#         self.upconv1 = nn.ConvTranspose2d(in_channels, hidden_dims[0], kernel_size=3, stride=2, padding=1, output_padding=1)
#         self.upconv2 = nn.ConvTranspose2d(hidden_dims[0], hidden_dims[1], kernel_size=3, stride=2, padding=1, output_padding=1)
#         self.upconv3 = nn.ConvTranspose2d(hidden_dims[1], hidden_dims[2], kernel_size=3, stride=2, padding=1, output_padding=1)
        
#         # Final convolution to map to the desired output prediction (e.g., 1 channel for binary mask)
#         self.conv = nn.Conv2d(hidden_dims[2], out_channels, kernel_size=1) 

#     def forward(self, x):
#         """
#         Forward pass of the decoder. Applies a series of transposed convolutions (upconvolutions) and activation functions.
#         """
#         x = F.relu(self.upconv1(x))  # Apply transposed convolution and activation
