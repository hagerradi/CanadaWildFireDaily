...
from src.models.frp.ConvLSTMCell import ConvLSTMCell
...
class ConvLSTM(nn.Module):
    ...
    def _check_kernel_size_consistency(self, kernel_size):
        """Check if kernel_size is a tuple or a list of tuples."""
        if not (isinstance(kernel_size, tuple) or
                (isinstance(kernel_size, list) and all([isinstance(elem, tuple) for elem in kernel_size]))):
            raise ValueError('`kernel_size` must be tuple or list of tuples')

    def _extend_for_multilayer(self, param, num_layers):
        """Extend the parameters to a list if we are using multiple layers."""
    ...
    def _init_hidden(self, batch_size, image_size):
        ...
        init_states = []
        # Automatically detect what device (CPU/GPU) the model is on
        device = next(self.parameters()).device
        h, w = image_size
        
        for i in range(self.num_layers):
            h_t = torch.zeros(batch_size, self.hidden_dim[i], h, w, device=device)
            c_t = torch.zeros(batch_size, self.hidden_dim[i], h, w, device=device)
            init_states.append((h_t, c_t))
