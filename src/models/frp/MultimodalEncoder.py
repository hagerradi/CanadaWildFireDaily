...
from src.models.frp.ConvLSTM import ConvLSTM
...
class StateBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x[-1] # return_all_layers is True Now --> grabbing the last layer
...
class DynamicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x, _ = self.convlstm(x)
        # Get the output of the last ConvLSTM layer
        # x = x[0]  # Assuming return_all_layers is False
        x = x[-1] # return_all_layers is True Now --> grabbing the last layer
...
#     ConstantBlock for processing spatial data using Conv layers.
#     """
#     def __init__(self, input_channels, hidden_channels=[64, 64, 64]):
#         super(ConstantBlock, self).__init__()
        
#         # Expanding the ~29 inputs up to 64 so we don't lose information
#         self.conv1 = nn.Conv2d(in_channels=input_channels, out_channels=hidden_channels[0], kernel_size=3, padding=1)
#         self.conv2 = nn.Conv2d(in_channels=hidden_channels[0], out_channels=hidden_channels[1], kernel_size=3, padding=1)
#         self.conv3 = nn.Conv2d(in_channels=hidden_channels[1], out_channels=hidden_channels[2], kernel_size=3, padding=1)
...
class CombinedBlock(nn.Module):
    ...
    def forward(self, x1, x2, x3):
        # Process each input through its respective block
        out1 = self.block1(x1)
        out2 = self.block2(x2)
        out3 = self.block3(x3)
        
        # Concatenate them along the channel dimension (dim=1)
        # out1, out2, and out3 must have the same spatial dimensions (Height, Width)
...
#     state_block = StateBlock(input_dim=1,
...
#     dynamic_block = DynamicBlock(input_channels=9,
#                         convlstm_hidden_channels=64,
#                         conv_hidden_channels=[64, 128, 128],
#                         device=device)

#     constant_block = ConstantBlock(input_channels=6,
