"""Script for the UMAMBA Model | Code from https://github.com/bowang-lab/U-Mamba/"""
...
if __name__ == "__main__":
    print("\n--- Testing UMambaEnc Initialization and Forward Pass ---")

    # Check GPU Availability
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Running test on device: {device}")

    # Parameters matching your spatial dataset
    batch_size = 2
    in_channels = 54
    height, width = 256, 256
    num_classes = 1

    # Instantiate UMambaEnc
    try:
        model = UMambaEnc(
        ...
        print("✔ UMambaEnc successfully initialized!")
    ...
    print(f"Dummy Input Shape: {dummy_input.shape} (Batch, Channels, H, W)")
    ...
    with torch.no_grad():
        try:
            ...
            if isinstance(output, (list, tuple)):
                print(f"Deep Supervision active. Returned {len(output)} outputs.")
                print(f"Main Output Shape: {output[0].shape}")
            else:
                print(f"Output Shape: {output.shape} (Expected: [{batch_size}, {num_classes}, {height}, {width}])")

            # Check if output requires squeezing for BCE loss (Batch, H, W)
            final_mask = output.squeeze(1) if not isinstance(output, (list, tuple)) else output[0].squeeze(1)
            print(f"Squeezed Output Shape for Loss: {final_mask.shape}")

            print("\n🎉 ALL TESTS PASSED SUCCESSFULLY! Model is ready for training.")
